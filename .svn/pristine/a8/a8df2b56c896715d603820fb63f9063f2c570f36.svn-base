...
- (void)initLayers{
    ...
    _trackColor = [UIColor colorWithWhite:0.5 alpha:1.0];
...
- (void)updateLayerFrames{
    ...
    _trackLayer.frame = CGRectInset(self.bounds, 0, [self thumbWidth]/2.2);
    [_trackLayer setNeedsDisplay];
...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event{
    ...
    if (_leftThumbLayer.highlighted) {
        ...
        self.leftValue = [self boundaryForValue:_leftValue minValue:_minValue maxValue:_rightValue-0.2];
    }else if (_rightThumbLayer.highlighted){
        self.rightValue += deltaValue;
        self.rightValue = [self boundaryForValue:_rightValue minValue:_leftValue+0.2 maxValue:_maxValue];
    }
    return YES;
}
