...
//    self.backgroundColor = [UIColor colorWithRed:70/255.0 green:70/255.0 blue:70/255.0 alpha:1.0];
    ...
    for (int k = 0; k < [self.strings count]; k++) {
        ...
        //加分割seg的图片
        CGFloat lineX = (kkViewWidth - SelectSwitchX * 2) / (self.strings.count) * k;
        UIImageView *black1lineImageView=[[UIImageView alloc] initWithFrame:CGRectMake(lineX, 0, 1, 31)];
        black1lineImageView.backgroundColor = [UIColor whiteColor];
        if (k != 0) {
            [self addSubview:black1lineImageView];
        }
    ...
    self.sliderView.autoresizingMask = YES;
...
//    if ([self.strings firstObject]||[self.strings lastObject]) {
//        self.sliderView.layer.cornerRadius = self.cornerRadius;
//    }else{
//        self.sliderView.layer.cornerRadius = 0;
//    }
    
    self.sliderView.layer.cornerRadius = self.cornerRadius-self.cornerRadius*3.0/5.0;
    ...
    CGFloat sliderWidth = (kkViewWidth - SelectSwitchX * 2) / [self.strings count];
...
//    self.sliderView.frame = CGRectMake(sliderWidth * self.selectedIndex, self.backgroundView.frame.origin.y, sliderWidth - self.sliderOffset, self.frame.size.height - self.sliderOffset);
...
- (void)animateChangeToIndex:(NSUInteger)selectedIndex
{
    ...
    [UIView animateWithDuration:0.25 delay:0.0 options:UIViewAnimationOptionCurveEaseIn animations:^{
        ...
        ZHLog(@" self.sliderView.frame.size.width  = %d",self.sliderView.frame.size.width);
...
//    [self animateChangeToIndex:self.selectedIndex];
    [self changeToIndexWithoutAnimation:self.selectedIndex];
}
