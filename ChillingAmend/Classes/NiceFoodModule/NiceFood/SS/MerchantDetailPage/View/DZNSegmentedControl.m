...
- (void)setShowsGroupingSeparators:(BOOL)showsGroupingSeparators
{
    ...
    for (int i = 0; i < [self buttons].count; i++) {
        ...
        [self configureButtonForSegment:i];
        //加分割seg的图片
        CGFloat lineX = SCREEN_WIDTH/([self buttons].count)*i;
        UIImageView *black1lineImageView=[[UIImageView alloc] initWithFrame:CGRectMake(lineX, (_height-20)/2, 1, 20)];
        black1lineImageView.tag = i+101;
        [black1lineImageView setImage:[UIImage imageNamed:@"msxq_0010_content_tab_line_small"]];
        if (i != 0) {
