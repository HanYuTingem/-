...
//        }
        ...
        list.oId  = [NSString stringWithFormat:@"%@",HUserId];
...
- (void)startDownLoad
{
    ...
    NSDictionary *parameter = @{@"oId":HUserId,@"proIden":PROIDEN,@"seatId":self.seatId};
...
- (void)startCancelBookSeatDownLoad
{
    [self showStartHud];
    NSString * str  = [NSString stringWithFormat:@"%@",NFM_URL];
    NSDictionary *parameter = @{@"oId":HUserId,@"seatId":[NSString stringWithFormat:@"%@",self.seatId],@"proIden":PROIDEN};//@"ownerId":@""
