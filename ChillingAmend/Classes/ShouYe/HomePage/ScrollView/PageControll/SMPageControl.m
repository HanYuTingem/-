...
//	CGContextFillRect(context, CGRectMake(left, 0, size.width, size.height));
	...
	for (NSUInteger i = 0; i < _numberOfPages; i++) {
		...
		if (i == _displayedPage) {
			fillColor = _currentPageIndicatorTintColor ? _currentPageIndicatorTintColor : [UIColor clearColor];
			image = _currentPageImages[@(i)];
			if (nil == image) {
				image = _currentPageIndicatorImage;
			}
		} else {
			fillColor = _pageIndicatorTintColor ? _pageIndicatorTintColor : [[UIColor clearColor] colorWithAlphaComponent:0.3f];
			image = _pageImages[@(i)];
			if (nil == image) {
		...
		if (image) {
            if (i == _displayedPage) {
                [image drawAtPoint:CGPointMake(xOffset - 2.5, yOffset - 2.5)];
            }else{
                [image drawAtPoint:CGPointMake(xOffset, yOffset)];
            }
            CGContextFillRect(context, CGRectMake(left, 0, 1, 1));
