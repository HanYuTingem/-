...
//    //    self.shareContent = [NSString stringWithFormat:@"%@商城里的%@，真心不错啊!快来看看吧! http://cqtv.sinosns.cn/",@"黑土",goods.name];
//    
...
//    
//    self.shareContent = [NSString stringWithFormat:@"%@商城里的%@，真心不错啊!快来看看吧! %@",@"黑土",goods.name,shareURL];
...
- (void)callOutShareGoodS:(LSYGoodsInfo*)goods
{
    ...
    self.shareContent = [NSString stringWithFormat:@"%@商城里的%@，真心不错啊!快来看看吧! %@",@"黔包",goods.name,shareURL];
    NSURL *imageUrl = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@",goods.host,goods.images[0][@"url"]]];
    UIImage *shareImage = [UIImage imageWithData:[NSData dataWithContentsOfURL:imageUrl]];
    self.shareImageName = shareImage;
}
...
- (void)callOutShareViewWithUseController:(UIViewController *)addController andSharedUrl:(NSString*)url
{
    ...
    [UMSocialData defaultData].extConfig.sinaData.shareText = [NSString stringWithFormat:@"%@ %@",@"来自黑土的分享",url];//微博
    ...
    if (url == nil) {
        url = @"http://ht.sinosns.cn/";
    }
    //    // 友盟分享  5328fbfa56240b9ada067458
    [UMSocialData setAppKey:@"565d3d56e0f55a169f0004c3"];
    // 设置是否打开log输出，默认不打开，如果打开的话可以看到此sdk网络或者其他操作，有利于调试
    [UMSocialData openLog:YES];
    // 设置sdk所有页面需要支持屏幕方向
    [UMSocialConfig setSupportedInterfaceOrientations:UIInterfaceOrientationMaskAll];
    //设置微信AppId，设置分享url，默认使用友盟的网址
    [UMSocialWechatHandler setWXAppId:@"wx68cbf500fcda3024" appSecret:@"ef37bd35b48296dfa2de86bb38841d43" url:url];
    //打开新浪微博的SSO开关
    [UMSocialSinaHandler openSSOWithRedirectURL:@"http://sns.whalecloud.com/sina2/callback"];
    
    //设置分享到QQ空间的应用Id，和分享url 链接
    [UMSocialQQHandler setQQWithAppId:@"1104996522" appKey:@"fLQqwcFnOaY026fi" url:url];
    //设置支持没有客户端情况下使用SSO授权
