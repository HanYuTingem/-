...
+ (NSData*)encryptAESData:(NSString*)string andPublicPassWord:(NSString *)passWord{
    NSData *data = [string dataUsingEncoding:NSUTF8StringEncoding];
    //使用密码对nsdata进行加密
...
+(NSString *)URLencryptAESData:(NSString *)jsonStr andPublicPassWord:(NSString *)passWord{
  NSData *data =   [self encryptAESData:jsonStr andPublicPassWord:passWord];// AES 加密
    NSString *url=  [SecurityUtil encodeBase64Data:data]; //BASE64 加密
   NSString *encoding = [WalletRequsetHttp encodeString:url];// 去掉非法字符
    return encoding;
}
#pragma mark - MD5加密
/**
